        ...
        public static void OnPlayerClick(Player p, MouseButton button, MouseAction action, ushort yaw, ushort pitch, byte entity, ushort x, ushort y, ushort z, TargetBlockFace face)
        {
            ...
            switch (button)
            {
                case MouseButton.Left:
                    PlayerEvents.PlayerUsingItemLeftClickEvent.Call(p, Items[bid], ref cancel);
                    if (!cancel) Items[bid].OnLeftClick(p, action, yaw, pitch, entity, x, y, z, face);
                    break;
                case MouseButton.Right:
                    PlayerEvents.PlayerUsingItemRightClickEvent.Call(p, Items[bid], ref cancel);
                    if (!cancel) Items[bid].OnRightClick(p, action, yaw, pitch, entity, x, y, z, face);
                    break;

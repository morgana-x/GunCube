...
namespace GunCube.Modules.Projectile
{
    public class Bullet : Projectile
    {
        ...
        public Bullet(Level level, Player thrower, Vec3F32 position, Vec3S32 velocity) : base(level, thrower, position, velocity)
        {
        ...
        public override bool Tick(float curtime)
        {
            for (int i = 0; i < 5; i++)
            {
                Effect.EmitEffect(Level, Effect.Effects.Bullet_Trail, Pos + (Vel*i*0.05f));
                Effect.EmitEffect(Level, Effect.Effects.Bullet_Trail2, Pos + (Vel*i*0.05f));
            }
        ...
        public override void OnCollide(ushort block, Player pl)
        {
            if (pl != null)
            {
                ...
                if (!cancel)
                {
                    pl.Send(MCGalaxy.Network.Packet.VelocityControl(Vel.X * 0.1f, 0.5f, Vel.Z * 0.1f, 0, 0, 0));
            ...
            Effect.EmitEffect(Level, Effect.Effects.Bullet_Hit, Pos + new Vec3F32(0, 1, 0));
